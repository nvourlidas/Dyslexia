...
type ColKey = "amka" | "code_diagnosis" | "end_date"
const COL_LABELS: Record<ColKey, string> = {
  amka: "ΑΜΚΑ",
  code_diagnosis: "Κωδ. Διάγν.",
  end_date: "Λήξη",
}
const ALL_COLS: ColKey[] = ["amka", "code_diagnosis", "end_date"]
...
export default function ParapemtikaExecutionModal({ open, tenantId, onClose, onCountChange }: Props) {
      ...
      <div className="flex min-h-full items-center justify-center p-4">
        <div className="relative w-full max-w-2xl rounded-2xl border border-border bg-panel shadow-xl">
          ...
          <div className="sticky top-0 z-10 flex items-center justify-between rounded-t-2xl border-b border-border bg-panel px-5 py-3">
            <div>
              <div className="font-semibold">Παραπεμπτικά προς εκτέλεση</div>
                            ...
                            <div className="flex items-start gap-3">
                              ...
                              <div className="min-w-0 flex-1 space-y-1">
                                ...
                                <div className="flex flex-wrap gap-x-4 gap-y-0.5 text-xs text-muted">
                                  {visibleCols.has("amka") && item.amka && (
                                    <span>ΑΜΚΑ: <span className="font-mono">{item.amka}</span></span>
                                  )}
                                  {visibleCols.has("code_diagnosis") && item.code_diagnosis && (
