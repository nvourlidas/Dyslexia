// supabase/functions/teacher-bulk-delete/index.ts
...
postHandler(async (payload, tenantId, _profile, req) => {
  ...
  try {
    await assertOwnedIds(admin, "teacher", ids, tenantId);
  } catch (err) {
  ...
  if (error) {
    if (error.code === "23503") {
      return fail(
        "CONFLICT",
        "Ένας ή περισσότεροι εκπαιδευτικοί έχουν συνδεδεμένες συνεδρίες και δεν μπορούν να διαγραφούν.",
        req,
        409,
      );
    }
